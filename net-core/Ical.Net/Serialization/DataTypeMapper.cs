    ...
    internal class DataTypeMapper
    {
        ...
        public bool GetPropertyAllowsMultipleValues(object obj)
        {
            ...
            return !string.IsNullOrWhiteSpace(property?.Name)
                && _propertyMap.TryGetValue(property.Name, out var mapping)
                && mapping.AllowsMultipleValuesPerProperty;
        }
        ...
        public Type GetPropertyMapping(object obj)
        {
            ...
            if (!_propertyMap.TryGetValue(property.Name, out var mapping))
            {
                return null;
            }

            return mapping.Resolver == null
                ? mapping.ObjectType
                : mapping.Resolver(property);
        }
    }
}